...
namespace Defra.Trade.CatchCertificates.Api.IntegrationTests.V3.Controllers.MmoProcessingStatementControllerTests;
...
public class UpsertTests : IClassFixture<CatchCertificatesApiWebApplicationFactory<Startup>>
{
    ...
    public UpsertTests(CatchCertificatesApiWebApplicationFactory<Startup> webApplicationFactory)
    {
        ...
        _webApplicationFactory.ApiVersion = "3-internal";
    ...
    public async Task Upsert_UnknownApplication_CreatesNew()
    {
        ...
        var (_, dtoToSave) = CreateProcessingStatementV3DataRow(_fixture, Version, "GBR-2023-CC-223UVWXYZ");

        using var content = new StringContent(JsonSerializer.Serialize(dtoToSave), null, "application/json");
        using var upsertResponse = await client.PostAsync($"mmo-processing-statement", content);
    ...
    public async Task Upsert_KnownApplication_UpdatesExisting()
    {
        ...
        var (existingDataRow, existingDto) = CreateProcessingStatementV3DataRow(_fixture, Version, "GBR-2023-CC-323UVWXYZ", null, dr => dr.LastUpdated = DateTimeOffset.UtcNow.AddMinutes(-1));
    ...
    public async Task Upsert_KnownApplicationButOldMessage_DoesNotUpdate()
    {
        ...
        var (existingDataRow, existingDto) = CreateProcessingStatementV3DataRow(_fixture, Version, "GBR-2021-CC-500ABCDEF", null, dr => dr.LastUpdated = DateTimeOffset.UtcNow.AddMinutes(-1));
    ...
    public async Task Upsert_InvalidPayload_BadRequest()
    {
        ...
        var (_, dtoToSave) = CreateProcessingStatementV3DataRow(_fixture, invalidVersion, null, d => { d.LastUpdatedSystem = "12345678901"; d.LastUpdatedBy = new string('a', 101); });

        using var content = new StringContent(JsonSerializer.Serialize(dtoToSave), null, "application/json");
        using var response = await client.PostAsync("mmo-processing-statement", content);
