...
namespace Defra.Trade.CatchCertificates.Api.IntegrationTests.V2.Controllers.MmoProcessingStatementControllerTests;
...
public class UpsertTests : IClassFixture<CatchCertificatesApiWebApplicationFactory<Startup>>
{
    ...
    public async Task Upsert_UnknownApplication_CreatesNew()
    {
        ...
        var (_, dtoToSave) = CreateProcessingStatementV2DataRow(_fixture, Version, "GBR-2023-CC-223UVWXYZ");
    ...
    public async Task Upsert_KnownApplication_UpdatesExisting()
    {
        ...
        var (existingDataRow, existingDto) = CreateProcessingStatementV2DataRow(_fixture, Version, "GBR-2023-CC-323UVWXYZ", null, dr => dr.LastUpdated = DateTimeOffset.UtcNow.AddMinutes(-1));
    ...
    public async Task Upsert_KnownApplicationButOldMessage_DoesNotUpdate()
    {
        ...
        var (existingDataRow, existingDto) = CreateProcessingStatementV2DataRow(_fixture, Version, "GBR-2021-CC-500ABCDEF", null, dr => dr.LastUpdated = DateTimeOffset.UtcNow.AddMinutes(-1));
    ...
    public async Task Upsert_InvalidPayload_BadRequest()
    {
        ...
        var (_, dtoToSave) = CreateProcessingStatementV2DataRow(_fixture, invalidVersion, null, d => { d.LastUpdatedSystem = "12345678901"; d.LastUpdatedBy = new string('a', 101); });
