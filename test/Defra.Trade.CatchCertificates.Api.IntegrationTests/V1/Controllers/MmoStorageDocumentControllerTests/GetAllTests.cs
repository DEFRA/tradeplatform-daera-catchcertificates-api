...
namespace Defra.Trade.CatchCertificates.Api.IntegrationTests.V1.Controllers.MmoStorageDocumentControllerTests
{
    public class GetAllTests : IClassFixture<CatchCertificatesApiWebApplicationFactory<Startup>>
    {
        ...
        public GetAllTests(CatchCertificatesApiWebApplicationFactory<Startup> webApplicationFactory)
        {
            ...
            _webApplicationFactory.StorageDocumentRepository
                ...
                .ReturnsAsync(
                    ...
                    {
                        return Tuple.Create(
                            _fixture.CreateMany<Models.StorageDocumentDataRow>((int)pageSize),
                            15L);
                    });
        }
        ...
        public async Task Get_WithPagination_Success()
        {
            ...
            var response = await client.GetAsync("mmo-storage-document?pageSize=10&pageNumber=1");

            var content = await response.Content.ReadAsAsync<PagedResult<StorageDocumentEntry>>();
