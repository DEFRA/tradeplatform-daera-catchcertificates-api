...
using Defra.Trade.CatchCertificates.Api.V3.Validation.Mmo;
...
namespace Defra.Trade.CatchCertificates.Api.Tests.V3.Validation.Mmo;
...
public class LandingValidatorTests
{
    ...
    public void Validate_Nulls_Error()
    {
        ...
        result.ShouldHaveValidationErrorFor(x => x.GearType)
           .WithErrorMessage("'Gear Type' must not be empty.");

        result.ShouldHaveValidationErrorFor(x => x.HighSeasArea)
        ...
        result.ShouldHaveValidationErrorFor(x => x.Rfmo)
            .WithErrorMessage("'Rfmo' must not be empty.");

        result.Errors.Should().HaveCount(24);
